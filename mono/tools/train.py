...
from mono.utils.do_test import *
...
def main(args):
    ...
    if not cfg.distributed:
        main_worker(0, cfg, args.launcher, train_data, test_data)
    else:
        # distributed training
        if args.launcher == 'ror':
            local_rank = cfg.dist_params.local_rank
            main_worker(local_rank, cfg, args.launcher, train_data, test_data)
        else:
            mp.spawn(main_worker, nprocs=cfg.dist_params.num_gpus_per_node, args=(cfg, args.launcher, train_data))
        
def main_worker(local_rank: int, cfg: dict, launcher: str, train_data: list, test_data: list):
    if cfg.distributed:
    ...
    for t in range(epochs):
        ...
        loss = train_loop(dataloader=train_data, model=model, loss_fn=loss_fn, optimizer=optimizer, cfg=cfg)
        if loss < best_loss:
            best_loss = loss
            model.save(model, 'best.pth')
...
# TODO: 
def train_loop(train_data, model, loss_fn, optimizer, cfg):
    ...
    model.train()
    for i, an in tqdm(enumerate(train_data)):
...
# def test_loop(train_data, model, loss_fn, optimizer, cfg):
#     model.train()
